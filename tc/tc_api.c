	...
	const u32 *fw_code, u32 fw_addr, size_t size)
{
	...
	size = TO_DWSZ(size);
	for (i = 0; i < size; i++) {
		if (cpu_to_be32(fw_code[i]) != tc_r32(fw_addr + TO_BYSZ(i))) {
			tc_err(to_tcpriv(priv), MSG_INIT,
		...
		const u32 *fw_code, size_t size)
{
	...
	for (i = 0, j = 0; i < size; i += 4, j++) {
		tc_w32(cpu_to_be32(fw_code[j]), code_base + i);
	}
		...
		struct sk_buff *skb, size_t size, u32 msg_type)
{
	...
	if (msg_type == MSG_TXDATA)
		pr_info("TX Data in driver:\n");
	else
		pr_info("RX Data in driver:\n");
...
void *tc_buf_alloc(void *priv, size_t size,
		u32 *phy_addr, enum tc_dir dir, struct device *pdev)
{
	void *buf;
	...
	buf = tcpriv->tc_ops.alloc(size, dir, &phyaddr);
...
void dfe_loopback_linkup(void *priv)
{
	...
	tc_dbg(tcpriv, MSG_EVENT, "disable idle keep only in emulation\n");
...
static int fw_md5_check(struct tc_priv *priv, const struct firmware *fw)
{
	...
	desc = kmalloc(sizeof(*desc) + crypto_shash_descsize(md5), GFP_KERNEL);
	if (!desc) {
		tc_err(priv, MSG_INIT, "Alloc md5 desc fail!\n");
		ret = -ENOMEM;
		goto __alloc_desc_fail;
	}
	...
	if (ret) {
		tc_err(priv, MSG_INIT, "md5 init fail!\n");
		goto __md5_fail;
	}

	ret = crypto_shash_update(desc, fw->data, data_len);
	if (ret) {
		tc_err(priv, MSG_INIT, "md5 update fail!\n");
		goto __md5_fail;
	}

	ret = crypto_shash_final(desc, output);
	if (ret) {
		tc_err(priv, MSG_INIT, "md5 final fail!\n");
		goto __md5_fail;
	}
...
int ppe_fw_load(struct tc_priv *priv)
{
	...
	if (fw_md5_check(priv, priv->fw.fw) < 0)
		return -EINVAL;
	/* get fw information */
