...
/* max 8M space */
#define MAX_FPI_ADDR	0x800000

static inline u32 mem_get_addr(struct tc_comm *priv,
			u32 addr, u32 off, u32 flag)
{
	switch (flag) {
	case SB_ADDR:
		return fpi_addr(addr) + TO_BYSZ(off);
	case SOC_ADDR:
	case FPI_ADDR:
		return addr + TO_BYSZ(off);
	default:
		return addr + TO_BYSZ(off);
	}
}

static inline u32 mem_get_val(struct tc_comm *priv, u32 addr, u32 off, u32 flag)
{
	switch (flag) {
	case SB_ADDR:
	case FPI_ADDR:
		return tc_r32(addr + TO_BYSZ(off));
	case SOC_ADDR:
		return readl((void *)(addr + TO_BYSZ(off)));
	default:
		return 0;
	}
}

static inline void mem_set_val(struct tc_comm *priv, u32 addr,
				u32 off, u32 val, u32 flag)
{
	switch (flag) {
	case SB_ADDR:
		sb_w32(val, addr + off);
		break;
	case SOC_ADDR:
		writel(val, (void *)(addr + TO_BYSZ(off)));
		break;
	case FPI_ADDR:
		tc_w32(val, addr + TO_BYSZ(off));
		break;
	default:
		return;
	}
				...
				size_t count, loff_t *data)
{
	int c, i;
	int action;
	u32 addr, set_val, repeat_cnt, rdaddr, val;
	struct tc_comm *priv;
	struct tc_priv *tcpriv;
	u32 addr_flag, prt_num;
	...
	if (vrx_strcmpi(param_list[idx], "s") == 0) {
		idx++;
		addr_flag = SOC_ADDR;
	} else if (vrx_strcmpi(param_list[idx], "f") == 0) {
		idx++;
		addr_flag = FPI_ADDR;
	}
	...
	if (addr >= MAX_FPI_ADDR && addr_flag != SOC_ADDR) {
		pr_info("Addr: 0x%x out of vrx518 internal memory space!\n",
			addr);
		return count;
	}
	if (action == PPE_PROC_READ) {
		...
		for (i = 0; i < repeat_cnt; i += prt_num) {
			...
			buflen = 0;
			rdaddr = mem_get_addr(priv, addr, i, addr_flag);
			sprintf(bufstr, "0x%08x: ", rdaddr);
			for (c = 0; c < prt_num && (i + c) < repeat_cnt; c++) {
				buflen = strlen(bufstr);
				val = mem_get_val(priv, rdaddr, c, addr_flag);
				sprintf(bufstr + buflen, "%08x  ", val);
			}
			pr_info("%s\n", bufstr);
		}
		...
		if (vrx_strcmpi(param_list[idx + 1], "r") == 0 ||
			...
			for (i = 0; i < repeat_cnt; i++)
				mem_set_val(priv, addr, i, set_val, addr_flag);
		} else {
			for (i = 0; i < num; i++) {
...
proc_mem_help:
	pr_info("echo <s|f> r [soc_addr] [count] > mem\n");
	pr_info("echo <s|f> w [addr] [value] [val0] <val1> ... > mem\n");
	pr_info("echo <s|f> w [addr] <value> r [repeat_count] > mem\n");
	return count;
}
...
proc_atm_prio_help:
	pr_info("echo pvc vpi.vci prio xx queue xx > prio\n");
	pr_info("echo pvc vpi.vci <add/del/flush> > prio\n");
	return count;
}
