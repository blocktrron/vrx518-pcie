...
static inline int pp32_is_stop(void *priv, int pp32)
{
	int f_stopped = 0;
...
static inline void aca_ring_addr_init(struct dc_ep_dev *ep_dev,
		struct ring_param *ring, u32 dir)
{
	u32 addr;

	addr = ep_dev->hw_ops->umt_msg_addr(ep_dev,
			ring->soc_endian, dir);
	ring->aca_cnt_addr = ep_dev->membase + addr;
