...
struct ring_param {
	unsigned int soc_endian;
	void *soc_dbase;
	unsigned int soc_phydbase;
	unsigned int soc_dnum;
	unsigned int soc_desc_dwsz;
	unsigned int soc_cnt_phyaddr;
	unsigned int prefill_cnt;
	unsigned int req_buf_num;
	void *aca_cnt_addr;
	unsigned int aca_cnt_phyaddr;
};

struct soc_cfg {
	struct ring_param txin;
	struct ring_param txout;
	struct ring_param rxin;
	struct ring_param rxout;
	int tx_fcs_en;
	int rx_fcs_en;
	int intr_en;
};
...
struct tc_hw_ops {
	...
	int (*subif_reg)(struct net_device *pdev, const char *dev_name,
...
extern int ptm_tc_load(struct tc_priv *, u32, enum dsl_tc_mode);
