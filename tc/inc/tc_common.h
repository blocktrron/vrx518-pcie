...
extern int (*ppa_drv_get_dslwan_qid_with_vcc_hook)(struct atm_vcc *);
...
extern int32_t (*ppa_drv_hal_get_mpoa_type_hook)(uint32_t dslwan_qid,
	uint32_t *mpoa_type);
extern int g_smartphy_port_num;
struct flag_header;
extern int (*g_smartphy_push_fn)(struct sk_buff *, struct flag_header *,
	unsigned int);
extern void xet_phy_wan_port(uint32_t port, uint32_t *flags,
	uint32_t wanmode, int xmode);
extern int32_t (*ppa_hook_ppa_phys_port_add_fn)(char *ifname, uint32_t port);
extern void (*ppa_hook_ppa_phys_port_remove_fn)(uint32_t port);
extern int32_t (*ppa_drv_datapath_mac_entry_setting)(uint8_t  *mac,
	uint32_t fid, uint32_t portid,
	uint32_t agetime, uint32_t st_entry , uint32_t action) ;
